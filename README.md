...
- **微信提醒**（可选）：每次成功填报即时推送提醒，服务异常时推送告警；优先走自建微信推送服务器（微信测试号模板消息），未配置时回退 Server酱
...
| WECHAT_PUSH_API_URL | 可选，自建微信推送服务器地址（POST /api/v1/send），配置后优先发送填报成功提醒与异常告警 | - |
| WECHAT_PUSH_API_TOKEN | 可选，自建微信推送服务器 Bearer Token | - |
| SERVERCHAN_SENDKEY | 可选，Server酱推送（自建推送未配置或失败时回退） | - |
...
配置自建微信推送服务器（WECHAT_PUSH_API_URL/TOKEN，微信测试号模板消息）或 Server酱（SERVERCHAN_SENDKEY）后，系统推送两类消息：

**填报成功提醒**：每次成功填报（实际产量 / 能源局产销量 / 实际销量，含追加、覆盖）后推送，包含煤矿、日期、填报人、产量/销量等概要；未配置任何推送通道时静默跳过。
...
每条告警包含错误类型、发生时间，异常场景附带调用栈；填报成功提醒仅包含填报概要，不泄露敏感业务数据。
