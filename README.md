...
本地调试通过后**同步到小主机（无 Docker）**：见 [docs/DEPLOY_SYNC.md](docs/DEPLOY_SYNC.md)（默认推送到 **[Gitee](https://gitee.com/mvpwanhao/ykmymanager)**，可选 `git push github main` 镜像到 [GitHub](https://github.com/mvpwanhao/ymkymanager)；服务器 `git pull` + `systemctl restart ymky`）。
