...
- **数据可视化图表过滤零数据煤矿**：柱状图和占比图中，所选区间内所有指标均为零的煤矿不再显示
  - 产量柱状图：period_prod 和 year_prod 均为 0 的煤矿隐藏
  - 销量柱状图：week_sales、month_sales、year_sales 均为 0 的煤矿隐藏
  - 产量占比饼图：period_prod 为 0 的煤矿隐藏
  - 销量占比饼图：已有过滤逻辑，无需改动
