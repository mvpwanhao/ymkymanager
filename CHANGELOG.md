...
- **每次成功填报微信提醒**：实际产量、能源局产销量、实际销量提交成功后，通过自建微信推送服务器（`WECHAT_PUSH_API_URL`/`WECHAT_PUSH_API_TOKEN`，微信测试号模板消息）推送「填报成功」提醒，含煤矿、日期、填报人、产量/销量等概要；未配置或发送失败时回退 Server酱（`SERVERCHAN_SENDKEY`），未配置推送通道时静默跳过。
...
- **周报表和产销量简报支持自定义日期区间**：两个卡片合并为一个「周报表及产销量简报」卡片，选择起始日期和结束日期后，一键同时生成周报表 Excel 和文本简报。默认每周起始日为周六。

### 变更

- **周报表、产销量简报从单日期改为日期区间选择**：`generate_weekly_report` 和 `generate_brief_report` 参数从 `target_date` 改为 `start_date, end_date`；前端表单从单个日期输入改为起始/结束双日期输入框。

### 修复

- **浅色模式日历按钮不可见**：`:root` 添加 `color-scheme: light`，浏览器原生日期选择器按钮在浅色模式下恢复正常显示。
- **简报周数计算错误**：周序号改为基于日历月（从1日起算）而非统计月（从26日起算）。例如 7 月 17 日原来显示"七月第4周"，修正为"七月第3周"。
- **不选煤矿时无法填报年累计掺配煤/外购煤**：`submit_sales` 重构，不选煤矿时若年累计掺配煤或外购煤任一非零则允许提交，仅更新"合计"记录的 I/J 值，跳过矿级销量录入。

### 优化

- **周报表 I5:J10 单元格留空不填 0**：吨表和万吨表的 I5:J10（各矿行）不再写入 0，改为显式清空；合计行 I11/J11 仍正常从"合计"记录取值。
- **简报数据格式调整**：所有数值去除千位分隔符，统一保留两位小数（含万吨数据和环比增量）。
- **数据可视化 KPI 标签重排**：第一行放置"年度实际原煤产量"和"年累计自产煤销量"（全宽并突出显示）；第二行放置"月累计自产煤销量""掺配煤年累计""外购煤年累计""合计销售煤量"；移除原第三行四个能源局相关标签。
- **台账表格列宽可拖拽调整**：所有 `.data-table` 表格表头右侧添加拖拽手柄，鼠标拖动即可调整列宽（最小 50px），手柄 hover 时显示蓝色高亮。
- **台账时间显示简化**：当年数据省略年份（`07-20 09:30`），跨年数据保留完整格式（`2025-07-20 09:30`）；管理员编辑保存时简短格式自动补全年份。
- **代码审查与重构优化**：根据代码审查报告完成全量优化。
  - 消除 `exclude_mines` 重复代码：`viz_engine` 和 `report_engine` 统一从 `app.utils` 导入。
  - 缓存 `compute_asset_version()`：添加 `@lru_cache`，避免每次模板渲染重新计算 SHA1。
  - 统一 `LOG_FILE` 定义：`Settings` 新增 `log_file` 属性，`main.py`/`pages.py` 统一引用。
  - 简化台账保存循环：`admin.py` 中 `O(n*m)` 嵌套循环改为 `O(n+m)` dict 映射。
  - 消除 `get_settings()` 副作用：`os.environ` 设置移至 `lifespan`，`get_settings()` 回归纯函数。
  - 拆分 `pages.py` `home()` 路由：9 个独立渲染函数 + `_PAGE_HANDLERS` 字典调度，`home()` 从约 200 行减至约 30 行。
...
- **前端设计改版（极简 + 暗色优先 + 玻璃拟态 + Bento 概览）**：
  - 玻璃拟态设计系统：新增明暗两套玻璃令牌（`--glass-bg / --glass-border / --glass-blur / --glass-shadow / --glass-hi`）与 `.glass` 通用工具类；顶栏与宽屏侧边栏改为半透明毛玻璃（saturate + blur + 顶部高光线 + 阴影）。
  - 极光渐变背景增强：mesh 渐变透明度由 0.04–0.10 提升至 0.13–0.24，浅色叠加蓝/青/紫三色辉光、暗色同理，使玻璃层透出可见的极光质感（玻璃拟态「看得见」的前提）。
  - 全站内容卡片透玻璃化：`.card` 由实色块改为半透明玻璃（`--card-glass-bg`），极光从卡片背后透出；表格行仍用实色保证密集数据可读，登录页保持实色不受影响。
  - KPI 概览 Bento 化：首两张（年度产量、年度销量）以玻璃 + 品牌描边 + 品牌色顶条 + 较大数字做适度强调，宽屏各跨 2 列占满第一行；其余 7 张均匀呈现。
  - 图表面板玻璃化 + 内高光，全局圆角令牌微调（卡片/面板更现代）。

### 修复

- **年度视图产量柱状图冗余双柱**：年度视图下「各矿产量对比」每矿原显示两根等高柱——「年度产量」与「年累计产量」统计口径恒等。现仅在非年度视图保留双系列（本月 vs 年累计，对比有意义），年度视图只显示单系列「年度产量」。纯前端 `renderProdBar` 逻辑调整，未改数据口径。
